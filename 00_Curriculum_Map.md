...
**Exam version:** May 4, 2026 guide (current live version, verified June 12, 2026)
**Source of truth:** [Official Exam Guide PDF](https://www.databricks.com/sites/default/files/2026-03/databricks-certified-data-engineer-associate-exam-guide-may-4-2026.pdf) | [Certification page](https://www.databricks.com/learn/certification/data-engineer-associate)
...
- 7 domains (replaces the older 5-domain structure still shown on the certification webpage summary)
...
The official May 2026 PDF lists the 7 domains and objectives but **does not publish percentage weights** in the text I could retrieve. The percentages below are estimates consistent with each domain's objective count and with prior research in `Databricks_Suggested_Outline.md`. Treat them as planning guidance, not official figures. The relative ranking (Transformation ≥ Ingestion > Jobs > Governance > CI/CD ≈ Troubleshooting > Platform) is well supported by the objective distribution.

## Domain → Week Mapping

| Exam Domain | Approximate Importance | Covered In Week | Notes |
| ----------- | ---------------------- | --------------- | ----- |
| 1. Databricks Intelligence Platform | ~6–10% | Week 1 | Lakehouse architecture, Delta Lake, Unity Catalog hierarchy, compute selection (serverless vs. classic, SQL warehouse vs. all-purpose). Bridge from learners' intro training. Fully hands-on in Free Edition. |
| 2. Data Ingestion and Loading | ~20% | Week 2 | COPY INTO and Auto Loader hands-on via managed volumes. Lakeflow Connect standard/managed connectors and JDBC/REST patterns = concept + decision-framework only ("which ingestion method?" questions). Schema enforcement/evolution and JSON/nested data are testable. |
...
| All domains | 100% | Week 5 (optional) | Capstone end-to-end pipeline + full 45-question timed mock weighted to domain importance + remediation by domain. If running 4 weeks only: mock becomes take-home homework after Week 4. |
